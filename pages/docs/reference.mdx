...
Creates `.aide/`, six hooks, MCP config, rules files for both editors, `.aide/config.json` seeded with every public setting, `.aide/config-reference.md`. Adds `.gitignore` entries for `.aide/cache/` and `.aide/memories/preferences/personal/`. Both `init` and `init --update-rules` are idempotent for the rules-file regeneration; running them again writes the same template content.
