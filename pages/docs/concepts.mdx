...
| `area_context` | Decisions and context for a specific code area | "The billing module is event-sourced; new code appends events, never mutates balances directly" |
| `guidelines` | Team-wide or project-wide principles | "Public API responses use `{error: {code, message}}` envelopes for any non-2xx status" |
