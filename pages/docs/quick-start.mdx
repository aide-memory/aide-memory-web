...
Created .aide/, six hooks, MCP config, rules for Claude Code + Cursor + a Devin CLI skill.
...
aide-memory's MCP server only becomes available after the editor reads `.mcp.json` (Claude Code), `.cursor/mcp.json` (Cursor), or `.devin/config.json` (Devin CLI) at session start. These editors load the config once at startup.
...
- **Devin CLI**: start a fresh session so it picks up `.devin/config.json`. Confirm with `devin mcp list` (shows `aide-memory`) and `devin skills list` (shows `/aide-memory`).
...
aide-memory works with Claude Code, Cursor, and Devin CLI today. Install in any project:

```bash
aide-memory init
```

All three editors read the same `.aide/memories/` directory. A memory captured in Claude Code is available in Cursor and Devin CLI immediately, and vice versa.

## Next steps

- [Concepts](/docs/concepts), the editor-agnostic mental model
- [Configuration](/docs/configuration), tune prompting, recall, injection, and more
- [Reference](/docs/reference), the 7 MCP tools your agent calls plus all 13 CLI commands
- [Hooks](/docs/hooks), per-hook walkthrough
- [Supported Editors](/docs/supported-editors), per-editor table
