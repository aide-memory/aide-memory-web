...
- **Single shared store across editors.** Claude Code, Cursor, and Devin CLI read the same `.aide/` directory, so a memory captured in one tool is visible to the other tools' agents without per-editor configuration.
...
| **Editor support** | Claude Code, Cursor, Devin CLI (full hook + MCP wiring) | Claude Code, Cursor, Gemini CLI, OpenCode, OpenClaw | MCP-first (any MCP host), plus REST / CLI / webhooks / Ollama proxy |
