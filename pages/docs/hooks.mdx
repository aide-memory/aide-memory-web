...
Hooks are the core of aide-memory's capture system. The agent does not call them; the editor does. Capture happens because the editor invokes the hook at the right moment, not because anyone remembers to call a tool.
...
Fires after `git checkout` or `git pull` (branch checkout, not file checkout). Runs `MemorySync.syncFromGit()` to rebuild the SQLite cache from JSON memory files. Targets under 500ms for 100 files; runs in the background with a 2-second timeout; never blocks git operations.
