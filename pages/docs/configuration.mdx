...
## Telemetry (opt-in: nothing sent unless you turn it on)
...
**2. Anonymized event tallies to PostHog (opt-in via env var).** Out of the box, **no events are sent**. Until you set `AIDE_TELEMETRY=on` in your environment, aide-memory makes zero network calls for telemetry. When you do opt in, aide-memory sends anonymized event tallies (just counts, no content) so we can see which features are used.
...
**Stay opted out (the default):** do nothing. Without `AIDE_TELEMETRY=on`, aide-memory never makes a telemetry network call.
