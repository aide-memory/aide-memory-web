...
aide-memory ships with defaults aimed at a common-case workflow. Many parts of the flow are tunable: if you don't like one piece (chatty per-file recall, hard-block on first read, default-shared preferences, periodic Stop nudges), flip one knob.
