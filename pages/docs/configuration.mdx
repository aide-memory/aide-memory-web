...
| `hooks.correction.enabled` | boolean | `true` | Detect correction phrasings ("no, use X instead", etc.) in user messages and emit a soft hint via `hookSpecificOutput.UserPromptSubmit.additionalContext`. Master switch — when `false`, all other correction keys are inert. |
| `hooks.correction.escalate` | `"off" \| "block"` | `"off"` | When set to `"block"`, also write a `correction-pending-<sid>.txt` flag so the next `Stop` fire emits `decision:"block"` with a correction-not-stored reminder, regardless of `hooks.stop.mode`. Default `"off"` — correction surfaces only as the in-turn soft hint; no follow-up reminder. (A `"soft"` value existed in spec drafts but was dropped — Claude Code's `Stop` hook doesn't accept `hookSpecificOutput.additionalContext`, so a soft Stop emit isn't possible. The in-turn `UserPromptSubmit` hint IS soft and is what fires by default.) |
| `hooks.stop.mode` | `"block" \| "off"` | `"block"` | How the scheduled `Stop` checkpoint emits. `"block"` (default) uses `decision:"block"` + a softened reason ("Anything from this turn worth persisting…? Otherwise stop.") + chrome `systemMessage`. Claude Code's TUI collapses the "Stop hook error:" label behind ctrl+o when `systemMessage` is present, so the brand chrome is the primary visible line. `"off"` silences scheduled fires entirely (correction-pending escalation still works if `escalate = "block"`). |
...
| `telemetry.enabled` | boolean | `true` | Controls both local SQLite analytics and PostHog remote telemetry. Set `false` to disable all telemetry. The `AIDE_TELEMETRY=off` env var also works and takes precedence if set. |
