...
Yes. aide-memory is **proprietary freeware**: free to use today, no usage caps, no memory count limits, no feature gates for solo developers. The plan is to keep core memory features free for solo developers, add more free enhancements over time, and potentially layer paid team / pro features in the future. Source is not public; aide-memory is not open source, not FSL, not Apache 2.0, not MIT.
...
After `init`, **restart your editor** so it picks up the new MCP server (both editors read MCP config at session start, not live). On Cursor, also enable the aide-memory MCP server in **Settings → MCP** since Cursor disables newly-discovered MCP servers by default.
...
The one network surface is opt-in PostHog telemetry. Until you set `AIDE_TELEMETRY=on` in your environment, aide-memory makes **zero telemetry network calls**. When you do opt in, only anonymized event tallies are sent (event type, a SHA256-hashed `hostname:username` for deduplication, platform, Node version). Memory content, code, file paths, query strings, and any other user-identifying data are never sent.
...
claude-mem installs via `npx claude-mem install` and uses Chroma (vector DB) plus a Bun-managed worker service. aide-memory installs via `npx aide-memory init` and stores memories as JSON files plus a local SQLite cache, no extra runtime.

See the [Comparison page](/docs/comparison) for the full table.
...
aide-memory's sync logic resolves cache vs file disagreements by `updated_at` timestamp (newer wins), but for git merges, the file-level conflict is yours to resolve like any other text file.
...
aide-memory uses a different storage format (JSON files vs vector DB / typed stores). Manual migration is not yet supported. We recommend starting fresh with aide-memory in your project.

If you have specific use cases, file an issue on GitHub.

## Does aide-memory replace CLAUDE.md / .cursorrules / skills?

No. aide-memory complements them. CLAUDE.md and .cursorrules sit at the editor-config layer for static, project-wide guidance. aide-memory adds **dynamic, scoped, layered, auto-captured** memory on top. Both coexist. The `init` command writes its own rules files (`.claude/rules/aide-memory.md` and `.cursor/rules/aide-memory.mdc`) that teach the agent when to call each MCP tool; your existing rules files keep doing their job.

Skills / sub-agents still trigger as configured. The mental model: rules files cover always-on guidance, skills cover procedures, aide-memory covers the dynamic scoped knowledge that grows with the code. The three coexist; aide-memory doesn't sit above or below the others.

## Does aide-memory cost extra inference money?

No. aide-memory is a typed store + dispatcher + MCP server. It does **no LLM calls of its own**. The agent in the editor you already use does all the reasoning, no extra inference cost. The seven MCP tool descriptors fit comfortably in any agent's tool budget.
...
That's a real risk we're working to mitigate. Today, the rules file written at init nudges the agent to call `aide_remember` when corrections or decisions surface, and the Stop hook periodically prompts a reflection. Both help capture stay current. As use grows, we plan to add tooling to surface stale memories (low recall count, last-updated long ago) and prompt the team to re-evaluate or `aide_forget`. If you have a specific drift scenario you're worried about, file an issue.
...
Yes. aide-memory works in any project, git or not. Memories are stored locally in `.aide/`. You just won't get automatic team sync without git, and the `post-checkout` rebuild hook isn't installed.
