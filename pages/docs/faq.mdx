...
```bash
npx aide-memory config set hooks.visible false
```
...
```bash
npx aide-memory config set recall.minScopeDepth 2
npx aide-memory config set recall.limit 30
```
...
```bash
npx aide-memory config set hooks.read.maxBlocks 0
npx aide-memory config set hooks.visible false
```
