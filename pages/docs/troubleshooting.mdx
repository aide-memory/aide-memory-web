...
2. **MCP server not configured in your editor.** Ensure the MCP server is registered. For Claude Code, check `.mcp.json`. For Cursor, check `.cursor/mcp.json` and that the server is toggled ON in Cursor → Settings → MCP.
...
1. **Restart Cursor after `aide-memory init`.** Cursor reads `.cursor/mcp.json` on startup only; there is no hot-reload. Cmd+Q the Cursor app (don't just close the window) and reopen.

2. **Toggle the aide-memory MCP server ON.** Cursor → Settings → MCP. Newly-discovered servers stay disabled by default.
...
4. **Cursor uses different hook delivery.** Cursor uses `agent_message` for soft hook responses rather than `additionalContext`. The rules file template accounts for this.

See [Cursor editor page](/docs/editors/cursor) for the full gap list and Cursor-specific tracking.
