...
`aide-memory init` writes Claude Code's config files. **Restart your Claude Code session after init** so it picks up the new MCP server (Claude Code reads `.mcp.json` once at session start).
