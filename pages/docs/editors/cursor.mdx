...
2. **Toggle the aide-memory MCP server ON.** Cursor → **Settings** → **MCP**.
...
**When you type a correction.** The `beforeSubmitPrompt` hook detects correction patterns and, by default (`hooks.correction.escalate = "off"`), emits a soft in-turn hint via Cursor's `agent_message` channel. The agent sees the prompt as extra context and decides whether to call `aide_remember`. No follow-up reminder fires. Set `hooks.correction.escalate = "block"` to also write a `correction-pending-<sid>.txt` flag — the next Stop hook's `followup_message` then reminds the agent if no memory was stored.
