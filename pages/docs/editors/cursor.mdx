...
## Behavior we've noted, may improve as Cursor's platform evolves

- Native `sessionStart.additional_context` rendering for new sessions
- Native `additional_context` channel on `beforeSubmitPrompt` (would let correction reminders fire in-turn)
- Native session-start re-fire after compaction
- Inline rendering of `user_message` on `permission: "allow"` events (would put soft chrome in chat instead of the Hooks Output panel)
- MCP server name in `beforeMCPExecution` payload (would help disambiguate same-named tools across servers)

When Cursor ships any of these, aide-memory's adapter will switch to the native channel and the workaround retires.
