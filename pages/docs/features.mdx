...
- **Cross-editor** (Claude Code, Cursor + Devin CLI today)
...
- **Devin CLI**: full hook + MCP wiring (Devin's hooks are Claude-Code-compatible). `aide-memory init` writes a single `.devin/config.json` (hooks + MCP server) plus a `.devin/skills/aide-memory/SKILL.md` skill — all scoped to `.devin/`. Pre-tool nudges are soft (Devin's hard block is non-recoverable) and aide-memory activity shows via MCP tool-call cards rather than branded chrome. See the [Devin editor page](/docs/editors/devin).
