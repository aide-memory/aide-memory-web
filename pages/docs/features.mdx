...
- **PreToolUse**, blocks first read of a file with scoped memories until `aide_recall` runs; soft-nudges thereafter; configurable per-mode
...
- **Stop**, periodic "anything worth remembering?" reflection nudge on a configurable schedule (default: every 3 / 5 / 10 turns ramp)
- **PreCompact**, prompts the agent to save active plans/decisions before compaction; clears session tracking so post-compact reads re-prompt cleanly

## Recall (path-scoped + layered)
...
See [Supported Editors](/docs/supported-editors) for the full capability matrix.

## Configurable everything

- **6 hook controls** (`hooks.read.maxBlocks`, `hooks.edit.maxBlocks`, `hooks.search.mode`, `hooks.correction.enabled`, `hooks.precompact.mode`, `hooks.stop.schedule`, `hooks.visible`)
...
- Telemetry (anonymized event counts only) is **opt-in** via `AIDE_TELEMETRY=on`. Default is off.
